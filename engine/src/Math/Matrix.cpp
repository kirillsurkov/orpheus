...
Orpheus::Math::Matrix4 Orpheus::Math::Matrix4::perspective(float fov, float aspect, float near, float far) const {
    return Matrix4(glm::value_ptr(glm::perspective(fov, aspect, near, far)));
}

Orpheus::Math::Matrix4 Orpheus::Math::Matrix4::orthogonal(float left, float right, float bottom, float top) const {
